...
import {
  issueSheetCreatableStatusSchema,
...
const createIssueItemSchema = z.object({
  ...
  // Agent-created issues go through the same create path as everyone else: awaiting_verification
  // and verified are reached only via the real close/verify flow, never authored directly.
...
export function createCreateIssueTool(session: WorkspaceOrchestratorSession) {
  return defineAgentTool({
    ...
    execute: async ({ issues }) => {
      ...
      // createIssue's transition guard needs the actor's real membership role (only consulted
      // for the verify/reopen permission checks, which this tool's fields can never reach since
      // it has no verifierUserId input, but the type is required regardless).
      const [authorMembership] = await db
        ...
        .limit(1);
      if (!authorMembership) {
        throw new Error("automation_author_membership_not_found");
      }
      ...
      for (let index = created.length; index < issues.length; index += 1) {
        ...
        const record = await service.createIssue({
          ...
          projectId,
          actorUserId,
          actorRole: authorMembership.role,
          body: {
            title: issue.title,
